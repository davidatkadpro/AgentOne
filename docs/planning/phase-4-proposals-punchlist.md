...
|---|---|---|---|
| P0 Backend route gaps (P4P1-P4P10) | 0 | 0 | 10 |
| P1 List route (L1-L4) | 0 | 0 | 4 |
| P2 Detail shell (D1-D3) | 0 | 0 | 3 |
| P3 Estimate editor (E1-E4) | 0 | 0 | 4 |
| P4 Proposal preview (V1-V3) | 0 | 0 | 3 |
| P5 Generation flow (G1-G3) | 0 | 0 | 3 |
| P6 Revise + supersede (R1-R2) | 0 | 0 | 2 |
| P7 History + project tab (H1-H2) | 0 | 0 | 2 |
| P8 Polish + agent QA (Q1-Q3) | 0 | 0 | 3 |
| **Total** | **0** | **0** | **34** |
...
### R2. Supersede flow
**Status**: ☐ · **Depends on**: D2
- Toolbar `Supersede` (only on issued/accepted proposals) → dialog asking for the replacement proposal id (autocompleted from the same project's drafts).
- Confirm → `PATCH /api/proposals/:id { status: 'superseded', supersededByProposalId }`.
...
- Toolbar mounts `<AskAgentMenu module="proposals" tab="">` with skills like `Revise to total of $X`, `Draft cover letter`, `Re-extract from scope`.
- Spawned sessions stream inline below the toolbar (reuses `<InlineSessionStream>`).
