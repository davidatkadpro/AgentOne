...
export async function registerProjectsRoutes(
  ...
  for (const url of bothPaths('/:id/scope')) {
    app.get(url, async (req, reply) => {
      ...
      const inDir = safeJoin(storageRoot, `${project.folderPath}/in`)
      if (!inDir) {
        return { path: null, markdown: null, generatedAt: null }
      }
      let subdirs
  ...
  for (const url of bothPaths('/:id/files')) {
    app.get(url, async (req, reply) => {
      ...
      const rootAbs = safeJoin(storageRoot, project.folderPath)
      ...
      return { rootPath: rootAbs, entries }
    })
  }
...
/**
 * Join a relative path under an absolute root and reject path traversal. Used
 * by `/scope` and `/files` so a project's folder_path can't be poisoned with
 * `..` segments to read arbitrary files.
 */
